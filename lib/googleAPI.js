...
var SUBSCR_VAL = 'https://www.googleapis.com/androidpublisher/v3/applications/%s/purchases/subscriptionsv2/tokens/%s';
...
function validatePurchase(_googleServiceAccount, receipt, cb) {
    console.log("🚀 ~ validatePurchase ~ receipt:56", receipt)
    ...
    _getToken(googleServiceAccount.clientEmail, googleServiceAccount.privateKey, function (error, token) {
        ...
        console.log("🚀 ~ googleAPI.js:84 ~ token:", token)
        if (error) {
            return cb(error, {
                status: constants.VALIDATION.FAILURE,
        ...
        console.log("🚀 ~ googleAPI.js:102 ~ params:", JSON.stringify(params))
...
function _getToken(clientEmail, privateKey, cb) {
    ...
    request(params, function (error, res, body) {
        ...
        console.log("🚀 ~ googleAPI.js:165 ~ body:", body)
...
function _getValidationUrl(receipt, token) {
    ...
    switch (receipt.subscription) {
        case true:
            url = SUBSCR_VAL;
            console.log("🚀 ~ _getValidationUrl ~ url:", url)
            break;
        case false:
        default:
            url = PRODUCT_VAL;
